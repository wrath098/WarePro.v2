...
const isPrintSignatoryModalOpen = ref(false);
...
const openPrintSignatoryModal = (ppmpId, version) => {
    signatories.ppmpId = ppmpId;
    signatories.version = version;
    isPrintSignatoryModalOpen.value = true;

    const preparedByUser = props.users.find(u => u.name === 'Aileen G. Ramos');
    signatories.prepared_by = preparedByUser ? preparedByUser.id : '';

    const reviewer = props.users.find(u => u.name === 'Marjorie A. Bomogao');
    signatories.reviewed_by = reviewer ? reviewer.id : '';

    const approver = props.users.find(u => u.name === 'Jennifer G. Bahod');
    signatories.approved_by = approver ? approver.id : '';
};

const getUserById = (id) =>
    props.users?.find(u => u.id === id) || null

const preparedByPosition = computed(() => {
    const user = getUserById(signatories.prepared_by)
    return user?.position || 'No Position Available';
})

const reviewedByPosition = computed(() => {
    const user = getUserById(signatories.reviewed_by)
    return user?.position || 'No Position Available';
})

const approvedByPosition = computed(() => {
    const user = getUserById(signatories.approved_by)
    return user?.position || 'No Position Available';
})

const closePrintSignatoryModal = () => {
    isPrintSignatoryModalOpen.value = false;
    signatories.version = '';
    signatories.ppmpId = null;
};

const confirmPrint = () => {
    const url = route('generatePdf.ApprovedConsolidatedPpmp', {
        ppmp: signatories.ppmpId,
        version: signatories.version,
        preparedBy: signatories.prepared_by,
        reviewedBy: signatories.reviewed_by,
        approvedBy: signatories.approved_by,
    });

    window.open(url, '_blank');
    closePrintSignatoryModal();
};
...
onMounted(() => {
    ...
    if (page.props.auth?.user) {
        signatories.prepared_by = page.props.auth.user.id;
    }
    const reviewer = props.users.find(u => u.name === 'Marjorie A. Bomogao');
    if (reviewer) signatories.reviewed_by = reviewer.id;

    const approver = props.users.find(u => u.name === 'Jennifer G. Bahod');
    if (approver) signatories.approved_by = approver.id;
});
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <nav class="flex justify-between flex-col lg:flex-row" aria-label="Breadcrumb">
                ...
                <ol v-if="props.ppmp.status == 'Draft'">
                    <li v-if="hasPermission('consolidate-office-ppmp') || hasAnyRole(['Developer'])"
                        class="flex flex-col lg:flex-row justify-center items-center">
                        <button @click="showModal('copy')"
                            class="flex items-center rounded-md text-white bg-gradient-to-r from-indigo-500 via-indigo-600 to-indigo-700 hover:bg-gradient-to-br hover:text-gray-200 p-1 group">
                            <svg class="w-5 h-5 text-gray-100 group-hover:text-gray-200" fill="currentColor"
        ...
        <div class="my-4 w-screen-2xl bg-zinc-300 shadow rounded-md mb-8">
            <div class="overflow-hidden p-4">
                <div class="relative">
                    <DataTable class="display responsive table-hover table-striped bg-zinc-100" :columns="columns"
                        :data="transactions" :options="{
                            paging: true,
                            searching: true,
                            ordering: false,
                        }">
                        <template #action="props">
                            <div v-if="ppmp.status == 'Draft'">
                                <ViewButton v-if="hasPermission('view-app') || hasAnyRole(['Developer'])"
                                    :href="route('conso.ppmp.show', { ppmpTransaction: props.cellData.id })"
                                    tooltip="View" />
                                <IconButton v-if="hasPermission('edit-consolidation') || hasAnyRole(['Developer'])"
                                    @click="openEditModal(props.cellData)"
                                ...
                                <RemoveButton v-if="hasPermission('delete-app') || hasAnyRole(['Developer'])"
                                    @click="openDropPpmpModal(props.cellData)" tooltip="Trash" />
                            </div>
                            <div v-if="ppmp.status == 'Approved'" class="flex justify-center">
                                <Dropdown width="56">
                                        ...
                                        <button
                                            ...
                                            <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                                <g fill="currentColor">
                                                    ...
                                                    <path
                                                        d="M0 7a2 2 0 0 1 2-2h12a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2h-1v-2a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v2H2a2 2 0 0 1-2-2zm2.5 1a.5.5 0 1 0 0-1a.5.5 0 0 0 0 1" />
                                                </g>
                                            </svg>
                                            Print
                                    ...
                                    <template #content>
                                        <button @click="openPrintSignatoryModal(props.cellData.id, 'raw')"
                                            class="flex w-full px-4 py-2 text-start text-sm hover:bg-indigo-900 hover:text-gray-50">
                                            Raw File Format
                                        </button>
                                        <button @click="openPrintSignatoryModal(props.cellData.id, 'contingency')"
                                            class="flex w-full px-4 py-2 text-start text-sm hover:bg-indigo-900 hover:text-gray-50">
                                            Contingency Format
                                        </button>
                                    </template>
            ...
            <div class="bg-zinc-300 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="w-full mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-semibold text-[#1a0037]" id="modal-headline">Consolidate PPMP
                        </h3>
                        <p class="text-sm text-zinc-700"> Enter the details to generate a consolidated PPMP you wish to
                            consolidate.</p>
                        <div class="mt-5">
                            <p class="text-sm font-semibold text-[#1a0037]">PPMP Information: </p>
                            <div class="relative z-0 w-full my-3 group">
                                <select v-model="generateConsolidated.selectedType"
                                    @change="onTypeChange(generateConsolidated)" name="selectedType" id="selectedType"
                                    ...
                                    <option v-for="type in props.individualList" :key="type.ppmp_type"
                                        :value="type.ppmp_type">{{ type.ppmp_type == 'individual' ? 'Office' :
                                            type.ppmp_type }}</option>
                                </select>
                            ...
                            <div class="relative z-0 w-full my-3 group">
                                <select v-model="generateConsolidated.selectedYear"
                                    @change="onYearChange(generateConsolidated)" name="selectedYear" id="selectedYear"
                                    class="block py-2.5 px-2 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-700 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                    required>
                                    <option value="" disabled selected>Select Year</option>
                                    <option v-for="year in filteredYears" :key="year.ppmp_year" :value="year.ppmp_year">
                                        {{
                                            year.ppmp_year }}</option>
                                </select>
                            ...
                            <div class="w-full my-4">
                                ...
                                <div v-for="(account, index) in accountClass" :key="index"
                        ...
                        <div class="mt-5">
                            <p class="text-sm font-semibold text-[#1a0037]"> Price Adjustment: <span
                                    class="text-sm font-medium text-[#8f9091]">Increase the prices of a product item by
                                    a
                                    percentage.</span></p>
                            <div class="relative z-0 w-full group my-2">
                                <input v-model="generateConsolidated.priceAdjust" type="number" min="100" max="120"
                                    name="priceAdjust" id="priceAdjust"
                                ...
                                <label for="priceAdjust"
                                    ...
                                    must be within 100 to 120</label>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <form @submit.prevent="submitEditPpmp">
            <div class="bg-zinc-300 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="w-full mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-semibold text-[#1a0037]" id="modal-headline">Create Duplicate
                            of
                            Consolidated PPMP</h3>
                        ...
                        <div class="mt-5">
                            ...
                            <div class="relative z-0 w-full group my-2">
                                <input v-model="edit.ppmpDesc" type="text" name="ppmpDesc" id="ppmpDesc"
                                    class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-700 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                    placeholder="" required />
                                <label for="ppmpDesc"
                                    class="font-semibold text-zinc-700 absolute text-sm duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Enter
                                    description</label>
            ...
            <div class="bg-zinc-300 h-auto">
                <div class="p-6  md:mx-auto">
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-gray-600 my-2">Confirming this action will permanently remove the selected PPMP
                            including its particulars into the list. This action cannot be redo.</p>
    ...
    <Modal :show="isPrintSignatoryModalOpen" @close="closePrintSignatoryModal">
        <form @submit.prevent="confirmPrint">
            <div class="p-6 bg-indigo-100 rounded-md">
                <h3 class="text-lg font-semibold mb-4">Confirm Signatories</h3>
                <div class="lg:gap-6 my-5 mx-3">
                    <div class="bg-indigo-50 mb-5 rounded-xl p-2 shadow">
                        <div class="relative z-0 w-full my-2 group">
                            <label for="preparedBy" class="font-semibold text-zinc-700 text-md block mt-1r text-center">
                                Prepared By:
                            </label>
                            <label for="preparedBy" class="font-semibold text-zinc-700 text-xs block mt-1 mb-1">
                                Name:
                            </label>
                            <select id="preparedBy" v-model="signatories.prepared_by" class="block py-2.5 px-5 w-full text-sm bg-gray-100 rounded-xl border-0 border-b-2 border-t-2 border-gray-700 appearance-none
            focus:outline-none focus:ring-0 focus:border-blue-600 peer" required>
                            ...
                            <label class="font-semibold text-zinc-700 text-xs block mt-3">
                                Position
                            </label>
                            <input type="text" :value="preparedByPosition" disabled
                                class="block py-2.5 px-5 w-full text-sm bg-gray-100 rounded-xl border-0 border-b-2 border-t-2 appearance-none focus:outline-none cursor-not-allowed mt-1" />
                        </div>
                    </div>
                    <div class="bg-indigo-50 mb-5 rounded-xl p-2 shadow ">
                        <div class="relative z-0 w-full my-2 group">
                            <label for="reviewedBy" class="font-semibold text-zinc-700 text-md block mt-1r text-center">
                               Reviewed By:
                            </label>
                            <label for="reviewedBy" class="font-semibold text-zinc-700 text-xs block mt-1r mb-1">
                               Name:
                            </label>
                            <select id="reviewedBy" v-model="signatories.reviewed_by" class="block py-2.5 px-5 w-full text-sm bg-gray-100 rounded-xl border-0 border-b-2 border-t-2 border-gray-700 appearance-none
            focus:outline-none focus:ring-0 focus:border-blue-600 peer" required>
                                <option value="" disabled hidden></option>
                                <option v-for="user in users" :key="user.id" :value="user.id">
                                    {{ user.name }}
                            ...
                            <input type="text" :value="reviewedByPosition" disabled
                                class="block py-2.5 px-5 w-full text-sm bg-gray-100 rounded-xl border-0 border-b-2 border-t-2 appearance-none focus:outline-none cursor-not-allowed mt-1" />
                        </div>
                    </div>
                    <div class="bg-indigo-50 mb-5 rounded-xl p-2 shadow">
                        <div class="relative z-0 w-full my-2 group">
                            <label for="approvedBy" class="font-semibold text-zinc-700 text-md block mt-1r text-center">
                                Approved By:
                            </label>
                            <label for="approvedBy" class="font-semibold text-zinc-700 text-xs block mt-1r mb-1">
                                Name:
                            </label>
                            <select id="approvedBy" v-model="signatories.approved_by" class="block py-2.5 px-5 w-full text-sm bg-gray-100 rounded-xl border-0 border-b-2 border-t-2 border-gray-700 appearance-none
            focus:outline-none focus:ring-0 focus:border-blue-600 peer" required>
                                <option value="" disabled hidden></option>
                                <option v-for="user in users" :key="user.id" :value="user.id">
                                    {{ user.name }}
                            ...
                            <input type="text" :value="approvedByPosition" disabled
                                class="block py-2.5 px-5 w-full text-sm bg-gray-100 rounded-xl border-0 border-b-2 border-t-2 appearance-none focus:outline-none cursor-not-allowed mt-1" />
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6 flex justify-end space-x-2">
                    <DangerButton @click="closePrintSignatoryModal">Cancel</DangerButton>
                    <SuccessButton type="submit">Proceed</SuccessButton>
                </div>
            </div>
        </form>
    </Modal>
