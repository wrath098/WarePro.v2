...
const isAddPermissionModalOpen = computed(() => modalState.value === 'addPermission');
const isUpdateModalOpen = computed(() => modalState.value === 'update');
const isRemoveRoleModalOpen = computed(() => modalState.value === 'remove');
const isRemovePermissionModalOpen = computed(() => modalState.value === 'removePermission');
const isChangeModalOpen = computed(() => modalState.value === 'change');
...
const removeUserPermission = useForm({});
...
const removePermission = reactive({
    permission: '',
    userId: props.user.id,
});

const openRemovePermissionModal = (permission) => {
    removePermission.permission = permission;
    modalState.value = 'removePermission';
};
...
const submitPermission = () => {
    ...
    newPermission.post(route('user.assign.permission'), {
        onFinish: () => {
            isLoading.value = false;
        },
        onSuccess: () => {
...
const submitUpdatedInformation = async () => {
    updateUser.put(route('update.user.information'), {
        preserveScroll: true,
        onSuccess: () => {
            Swal.fire({
                title: 'Success',
...
const submitChangePassword = async () => {
    changePassword.put(route('user.new.password'), {
        preserveScroll: true,
        onSuccess: () => {
            Swal.fire({
                title: 'Success',
...
const submitRemovePermission = async () => {
    removeUserPermission.delete(route('user.revoke.permission', { param: removePermission }), {
        preserveScroll: true,
        onSuccess: () => {
            Swal.fire({
                title: 'Success',
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
                                <div v-for="role in roles" :key="role.id" class="flex items-center justify-between h-[50px] rounded-[5px] text-sm bg-indigo-500  w-full px-2 pl-4 font-light mb-2">
                                    ...
                                    <button
                                        ...
                                        class="text-lg text-rose-600 px-4 py-1 rounded-lg hover:bg-indigo-200 transition duration-300 shadow-sm"
                                    >
                                        X
                                    </button>
                                </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
                            <div v-for="(permission, index) in direct_permissions" :key="permission.index" class="flex items-center justify-between h-[50px] rounded-[5px] text-sm bg-teal-500  w-full px-2 pl-4 font-light mb-2">
                                ...
                                <button @click="openRemovePermissionModal(permission)" class="text-lg text-rose-600 px-4 py-1 rounded-lg hover:bg-teal-300 transition duration-300 shadow-sm">X</button>
                            </div>
                        </div>
                    </div>
                    <div class="mt-5 flex justify-end text-xs">
