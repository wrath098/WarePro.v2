        ...
        <div class="w-full mx-auto sm:mx-4 lg:mx-0 mt-4">
            <div class="w-full bg-zinc-300 h-auto mb-2 rounded-md shadow-md">
                <div class="bg-zinc-600 px-4 py-5 sm:px-6 rounded-t-lg">
                    ...
                    <p class="text-sm text-zinc-300">
                        Displays product issuances within the selected date range, including item details and release
                        information.
                    </p>
                </div>
                <form @submit.prevent="submitFilter">
                    <div class="grid lg:grid-cols-3 lg:gap-6 my-5 mx-3">
                        <div class="relative z-0 w-full my-5 group">
                            <input v-model="filterLogs.startDate" type="date" name="from" id="from"
                            ...
                            <label for="from"
                                class="font-semibold text-zinc-700 absolute text-sm duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">From
                                (Start Date)</label>
                        </div>

                        <div class="relative z-0 w-full my-5 group">
                            <input v-model="filterLogs.endDate" type="date" name="to" id="to"
                                class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-700 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                placeholder=" " required />
                            <label for="to"
                                class="font-semibold text-zinc-700 absolute text-sm duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">To
                                (End Date)</label>
                        </div>

                        <div class="relative z-0 w-full my-5 group flex justify-center items-center">
                            <button type="submit"
                                class="flex w-auto justify-center items-center mx-1 min-w-[100px] px-2 py-1 text-white transition-all bg-gray-600 rounded-md sm:w-auto hover:bg-gray-900 hover:text-white shadow-neutral-300 hover:shadow-2xl hover:shadow-neutral-400 hover:-tranneutral-y-px">
                            ...
                            <a :href="filteredLogs.data && filteredLogs.data.length > 0 ? route('generatePdf.ssmi', { filters: filterLogs }) : '#'"
                                target="_blank"
            ...
            <div class="bg-zinc-300 shadow-md sm:rounded-lg p-4">
                <div class="relative overflow-x-auto md:overflow-hidden">
                    <DataTable v-if="filteredLogs.length === 0"
                        class="display table-hover table-striped shadow-lg rounded-lg bg-zinc-100" :columns="columns"
                        :data="props.transactions" :options="{
                            paging: true,
                            searching: true,
                            ordering: true,
                        }" />
                    <DataTable v-if="filteredLogs.length !== 0"
                        class="display table-hover table-striped shadow-lg rounded-lg bg-zinc-100" :columns="columns"
                        :data="filteredLogs.data" :options="{
                            paging: true,
                            searching: true,
                            ordering: true
                        }" />
