...
class ApprovedOfficePpmpQuantityController extends Controller
{
    ...
    public function generatePdf_Ppmp(Request $request, PpmpTransaction $ppmp)
    {
        ...
        $table .= '</thead>';
        $table .= '<tbody>';
        $table .= $this->tableContent($ppmp, $totalQtySecond, $requestType);
        $table .= '</tbody>';
        $table .= '</table>';
    ...
    protected function tableContent($ppmp, $totalQtySecond, $requestType)
    {
        $text = '';
        $sortedParticulars = $this->formattedAndSortedParticulars($ppmp, $requestType);
